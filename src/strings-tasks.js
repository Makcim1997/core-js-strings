...
function sumOfCodes(str) {
  ...
  return str.split('').reduce((acc, e) => acc + e.charCodeAt(), 0);
}
...
function isPalindrome(str) {
  const str2 = [...str.toLowerCase().replace(/[^a-z]/gi, '')].reverse();
  return str2.join('') === str.toLowerCase().replace(/[^a-z]/gi, '');
}
...
function invertCase(str) {
  return str.replace(/([a-z])|([A-Z]+)/g, (_, low, up) => {
...
function encodeToRot13(str) {
  ...
  return str.replace(/[a-z]/gi, (lett) => cipher[originalAlpha.indexOf(lett)]);
}
 ...
 *   'K♠' => 51
 */
function getCardId(value, index) {
  return index;
}
